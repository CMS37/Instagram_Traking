...
const filterInstagramPosts = (edges, username, startDate, endDate, keywords) => {
	const rows = [];
	...
	for (const { node } of edges) {
		...
		if (!isPinned && ts <= startDate) { stopPaging = true; break; }
		newCount++;
		...
		relCount++;
		const likeCount = node.like_and_view_counts_disabled ? 'x' : node.edge_media_preview_like?.count ?? 'x';
		const commentCount = node.like_and_view_counts_disabled ? 'x' : node.edge_media_to_comment?.count ?? 'x';
		const viewCount = node.is_video ? (node.video_view_count ?? 'x') : 'x';
		rows.push([username, ts, `https://www.instagram.com/p/${node.shortcode}`, caption, viewCount, likeCount, commentCount]);
	}
...
const filterTikTokPosts = (items, username, startDate, endDate, keywords) => {
	...
	for (const item of items) {
		...
		if (ts <= startDate && !item.isPinnedItem) { stopPaging = true; break; }
		newCount++;
		if (ts < startDate || ts > endDate) continue;
